...
// Matches the fallback trigger format: <prefix>_<6 hex chars>. Used as a
// safety net for OAuth users created before the `nickname_generated`
// metadata flag was introduced.
const AUTO_GENERATED_NICKNAME_PATTERN = /_[0-9a-f]{6}$/;
...
const readUserMetadataBoolean = (user: User, key: string): boolean => {
  const metadata = user.user_metadata as Record<string, unknown> | null;
...
const toAccount = (user: User | null): AppAccount | null => {
  ...
  const nickname = readUserMetadataString(user, "nickname", "user_name", "username");
  const generatedFlag = readUserMetadataBoolean(user, "nickname_generated");
  const looksAutoGenerated =
    nickname.length > 0 && AUTO_GENERATED_NICKNAME_PATTERN.test(nickname);
  return {
    id: user.id,
    email: user.email,
    ...
    nickname,
    nicknameGenerated: generatedFlag || looksAutoGenerated,
  };
};
...
export const isOAuthProfileComplete = (account: AppAccount): boolean =>
  account.nickname.trim().length > 0 && !account.nicknameGenerated;
...
export const completeOAuthProfile = async (input: {
  ...
  // Atomically update user_profiles + auth metadata and clear the
  // `nickname_generated` flag server-side.
  const { error: rpcError } = await supabase.rpc("complete_oauth_profile", {
    new_nickname: normalizedNickname,
    new_first_name: normalizedFirst,
    new_last_name: normalizedLast,
  });

  if (rpcError) {
    const msg = (rpcError.message ?? "").toLowerCase();
    if (rpcError.code === "23505" || msg.includes("nickname already in use")) {
      return { ok: false, code: "nickname_exists" };
    }
  ...
  // Force a client-side refresh of the user record so the updated metadata
  // (including the cleared `nickname_generated` flag) is available locally.
  const { data, error: refreshError } = await supabase.auth.refreshSession();
  if (refreshError || !data.user) {
    const { data: userData, error: userError } = await supabase.auth.getUser();
    if (userError || !userData.user) return { ok: false, code: "unknown" };
    const account = toAccount(userData.user);
