...
const LidoModalCardComponent = ({
  ...
  const mapsLink = hasCoords
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(beach.name)}&center=${beach.lat},${beach.lng}`
    : null;
      ...
      >
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 pb-6 pt-4">
          ...
          <div className="rounded-[12px] border border-white/15 bg-black/30 p-4 text-sm br-text-primary backdrop-blur-sm">
            ...
            {beach.hasRoughSea ? (
              <div className="mt-3 flex items-center justify-between text-[11px] uppercase tracking-[0.12em] br-text-tertiary border-t border-white/5 pt-3">
                <span>{STRINGS.labels.roughSea}</span>
                <span className="text-[11px] font-semibold br-text-primary">
                  {STRINGS.report.presenceDetected}
                </span>
              </div>
            ...
            {beach.hasStrongWind ? (
              <div className="mt-3 flex items-center justify-between text-[11px] uppercase tracking-[0.12em] br-text-tertiary border-t border-white/5 pt-3">
                <span>{STRINGS.labels.strongWind}</span>
                <span className="text-[11px] font-semibold br-text-primary">
                  {STRINGS.report.presenceDetected}
                </span>
              </div>
...
const lidoModalEqual = (prev: LidoModalCardProps, next: LidoModalCardProps) => {
  ...
  return (
    ...
    a.hasRoughSea === b.hasRoughSea &&
    a.hasStrongWind === b.hasStrongWind &&
